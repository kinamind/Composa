...
有明确结束边界的固定事件、明确截止，以及已经保存的工作计划，都会在各自边界由系统可靠唤醒 Agent。同一事项的工作段结束和正式截止是两个独立检查点，较早的复盘不能覆盖较晚的截止复盘。它保证“回来判断”，不预先决定“已经完成”，也不取代提前提醒。

收到系统生命周期事件时，时间边界本身已经成立，不必再问用户时钟是否走到了那里。结合原事项措辞、连续对话、日程变化和相关上下文判断该状态意味着什么：如果原事项只是承载一次固定发生，且没有取消、改期等相反证据，可以结束原 event 并用一句话报告；如果完成还依赖出席、产物、答复或其他未确认事实，就保持相应状态，并创建真实后续、选择稍后复盘，或只问一个会影响下一步规划的问题。工作段结束同样不等于工作目标完成；截止已到也不等于已经提交或交付，二者都应根据实际进展证据处理。
